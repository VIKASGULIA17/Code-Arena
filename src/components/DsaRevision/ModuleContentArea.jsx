  ...
  return (
    ...
    >
      ...
      <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-3">
        Welcome to <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-purple-600">DSA Revision</span>
      </h1>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 w-full max-w-md">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-sm font-bold text-blue-600">{overall.percent}%</span>
        </div>
        ...
        <span className="text-xs text-gray-400">{overall.done} of {overall.total} items completed</span>
      </div>

      {/* Quick module cards */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-8 w-full max-w-2xl">
        {Object.entries(dsaCategories).map(([catId, category]) => (
          <div key={catId} className="bg-white rounded-xl border border-gray-200 p-4 text-left">
            <h3 className="font-semibold text-gray-900 text-sm mb-1">{category.title}</h3>
            <p className="text-xs text-gray-400">{Object.keys(category.topics).length} modules</p>
          </div>
        ))}
      </div>
    </motion.div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 md:p-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-gray-50 rounded-xl p-4 text-center border border-gray-100">
            <Code2 className="mx-auto mb-1.5 text-blue-500" size={22} />
            <div className="text-lg font-bold text-gray-900">{Object.keys(topic.codeTemplates || {}).length}</div>
            <div className="text-xs text-gray-400">Templates</div>
          </div>
          <div className="bg-gray-50 rounded-xl p-4 text-center border border-gray-100">
            <Zap className="mx-auto mb-1.5 text-amber-500" size={22} />
            <div className="text-lg font-bold text-gray-900">4</div>
            <div className="text-xs text-gray-400">Languages</div>
          </div>
          <div className="bg-gray-50 rounded-xl p-4 text-center border border-gray-100">
            <Target className="mx-auto mb-1.5 text-purple-500" size={22} />
            <div className="text-lg font-bold text-gray-900">{progress.percent}%</div>
            <div className="text-xs text-gray-400">Completed</div>
          </div>
          <div className="bg-gray-50 rounded-xl p-4 text-center border border-gray-100">
            <Clock className="mx-auto mb-1.5 text-green-500" size={22} />
            <div className="text-lg font-bold text-gray-900">{subtopics.length}</div>
            <div className="text-xs text-gray-400">Subtopics</div>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="font-bold text-blue-600">{progress.done}/{progress.total}</span>
          </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div className="divide-y divide-gray-100">
          {subtopics.map((sub, index) => (
            ...
            >
              <div className="w-8 h-8 rounded-lg bg-gray-100 flex items-center justify-center text-sm font-bold text-gray-500 group-hover:bg-blue-100 group-hover:text-blue-600 transition-colors">
                {index + 1}
              </div>
              <div className="flex-1">
                <span className="text-sm font-medium text-gray-800 group-hover:text-blue-600 transition-colors">{sub.label}</span>
                <span className="block text-xs text-gray-400 mt-0.5 capitalize">{sub.type === 'theory' ? 'Theory & Concepts' : 'Code Template'}</span>
              </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 text-sm text-gray-400 flex-wrap">
        ...
        <span>{category.title}</span>
        <ChevronRight size={14} />
        <span className="hover:text-blue-500 cursor-pointer" onClick={() => onSelectSubtopic(catId, topicId, null, null)}>{topic.title}</span>
        <ChevronRight size={14} />
        ...
        {subtopic.type === 'template' && topic.codeTemplates?.[subtopic.id] && (
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-gray-100">
              <h3 className="text-xl font-bold text-gray-900 mb-2">{topic.codeTemplates[subtopic.id].title}</h3>
