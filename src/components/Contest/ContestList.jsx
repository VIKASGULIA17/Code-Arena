import { CalendarDays, Grid3X3, ChevronLeft, ChevronRight, RotateCcw } from "lucide-react";
        ...
        {["All", "Ongoing", "Upcoming", "Finished"].map((status) => (
          <Button
            ...
            onClick={() => {
              ...
              else updateQuery("status", filters.status === status ? "All" : status);
            }}
          ...
          >
            {status === "Ongoing" && <div className="w-2 h-2 rounded-full bg-green-400 animate-pulse"></div>}
            {status}
          </Button>
        ))}
      </div>
      ...
      <div className="min-h-[50vh]">
        ...
        {filteredContest.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-64 bg-white rounded-2xl shadow-sm border border-dashed border-gray-300">
            <p className="text-gray-500 text-lg mb-4">No contests found matching your criteria.</p>
            <Button 
          ...
          <>
            ...
            {currentData.map((obj, idx) => (
              <div key={idx} className="my-4">
                <div className="w-full flex items-center justify-between h-auto rounded-2xl shadow-lg bg-white py-7 px-8 hover:scale-[1.01] transition-transform">
                  <div className="flex">
                    <div className="bg-gray-100 w-18 h-18 text-center pt-2 rounded-2xl flex flex-col justify-center px-4">
                      <p className="font-medium text-gray-600 uppercase text-xs">{obj.month}</p>
                      <p className="font-bold text-xl">{obj.date}</p>
                    </div>
                    <div className="ml-4">
                      <div className="flex gap-5 py-1 items-center">
                        <h1 className="text-2xl font-medium text-gray-700">{obj.name}</h1>
                        <span className={`px-4 py-1 rounded-full text-xs text-white font-bold bg-linear-to-b ${obj.color}`}>
                          {obj.status}
                        </span>
                      </div>
                      <div className="flex items-center gap-4 mt-2">
                        <div className="text-gray-500 text-sm">
                          ...
                          <span className="font-medium text-gray-700">{obj.startTime}</span>
                        </div>
                        <div className="w-1 h-1 bg-gray-300 rounded-full"></div>
                        <h4 className="text-gray-500 text-sm">{obj.duration} Duration</h4>
                      </div>
                    </div>
                  </div>
                  <div className="flex gap-4">
                    ...
                    <Button className="px-8 bg-brand-gradient font-bold">Virtual </Button>
                    :
                    <Button className="px-8 bg-brand-gradient font-bold">Register</Button>
                    }
                  </div>
                </div>
              </div>
